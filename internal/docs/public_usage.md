...
本服务已部署在 `https://cdnproxy.some.im`。使用时，只需将原始 URL 中的 `https://` 替换为 `https://cdnproxy.some.im/` 即可。
...
**通过本代理访问:**
```
https://cdnproxy.some.im/unpkg.com/react@18/umd/react.development.js
```

**使用方法：** 将原始 CDN URL 中的 `https://` 替换为 `https://cdnproxy.some.im/` 即可。
...
**通过代理:**
```
https://cdnproxy.some.im/api.anthropic.com/v1/messages
```
