...
将原始 API URL 中的 `https://` 替换为 `https://cdnproxy.facev.app/` 即可。

**格式：**
```
原始: https://api.example.com/v1/endpoint
代理: https://cdnproxy.facev.app/api.example.com/v1/endpoint
```
...
# 方法1: 修改 api_base
openai.api_base = "https://cdnproxy.facev.app/api.openai.com/v1"
...
client = OpenAI(
    ...
    base_url="https://cdnproxy.facev.app/api.openai.com/v1"
)
...
const configuration = new Configuration({
  apiKey: 'YOUR_API_KEY',
  basePath: 'https://cdnproxy.facev.app/api.openai.com/v1'
});
...
client = anthropic.Anthropic(
    api_key="YOUR_API_KEY",
    base_url="https://cdnproxy.facev.app/api.anthropic.com"
)
...
```bash
curl https://cdnproxy.facev.app/generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=YOUR_API_KEY \
  -H "Content-Type: application/json" \
  -d '{
    "contents": [{
      "parts": [{
...
```bash
curl https://cdnproxy.facev.app/api.groq.com/openai/v1/chat/completions \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -d '{
...
const ws = new WebSocket('wss://cdnproxy.facev.app/poe.com/api/chat', {
  headers: {
