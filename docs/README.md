...
- **硬盘缓存**: 对 `GET` 和 `HEAD` 请求的响应进行缓存，大幅提升重复访问的速度，减少对源 CDN 的请求。缓存时间可配置（默认 12 小时）。
- **访问控制**: 内置灵活的访问控制策略，防止代理服务被滥用。策略包括 User-Agent、Referer IP 和 Referer 域名白名单等。
- **管理后台**: 提供一个简单的 Web 界面，用于管理 Referer 白名单和查看基本设置。

### API 代理功能 (v2.1 新增)
- **AI API 代理**: 支持代理 OpenAI、Claude、Poe 等 AI 服务的 API 请求，解决国内无法访问的问题。
- **实时通信**: 支持 WebSocket 和 SSE (Server-Sent Events) 流式响应。
- **不缓存**: API 响应不会被缓存，确保每次都是实时数据。
- **大数据支持**: 支持 POST 大量数据，无大小限制。
- **长连接**: 支持长时间连接，无超时限制。
- **无访问控制**: API 请求不受 Referer 白名单限制，由上游 API 服务自己控制访问权限。
...
| `API_DOMAINS`         | 额外的 API 域名（逗号分隔）。            | 空（使用默认列表）                                  |
...
**特性说明：**
- ✅ 支持 POST/PUT/DELETE 等所有 HTTP 方法
- ✅ 支持 WebSocket 连接
